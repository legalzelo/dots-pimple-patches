...
export const metadata = {
  title: 'Dots — Parche anti-espinillas',
  description: 'El parche que reduce espinillas visiblemente en horas. Ingredientes seguros, resultados reales.',
  openGraph: {
    title: 'Dots — Parche anti-espinillas',
    description: 'El parche que reduce espinillas visiblemente en horas.',
    images: [{ url: '/og.jpg' }],
  },
};
...
export default async function LandingPage() {
  ...
  if (!product) {
    return <main className="mx-auto max-w-4xl p-8">No encontré el producto. Revisa SINGLE_PRODUCT_HANDLE en el .env.</main>;
  }

  const firstVariant = product.variants?.edges?.[0]?.node;
  const price = firstVariant?.price?.amount;
  const currency = firstVariant?.price?.currencyCode;
  const canBuy = !!firstVariant?.availableForSale;
  ...
  return (
    <main className="min-h-screen">
        ...
        <div className="grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <div className="mt-6 flex items-center gap-4">
              {canBuy ? (
                <BuyNow variantId={firstVariant.id} label={`Comprar ahora — ${price} ${currency}`} />
              ) : (
                <button className="rounded-2xl px-6 py-3 bg-gray-300 text-gray-600 cursor-not-allowed">Agotado</button>
              )}
      ...
      {/* PROBLEMA → SOLUCIÓN */}
      <Section eyebrow="¿Por qué salen?" title="Espinillas que aparecen cuando menos lo esperas" subtitle="Protege, aísla y acelera la recuperación con Dots">
      ...
      {/* CÓMO FUNCIONA */}
      <Section eyebrow="Cómo funciona" title="3 pasos simples">
        <div className="grid md:grid-cols-3 gap-6">
          {[
      ...
      {/* INGREDIENTES */}
      <Section eyebrow="Ingredientes" title="Lo que hace la diferencia" subtitle="Fórmula efectiva, amable con tu piel">
        <div className="grid md:grid-cols-3 gap-6">
          {[
      ...
      <Section eyebrow="Resultados reales" title="Antes y después">
      ...
      {/* GARANTÍA */}
      <Section eyebrow="Sin riesgo" title="Garantía 30 días">
